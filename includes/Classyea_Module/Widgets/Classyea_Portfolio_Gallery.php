...
class Classyea_Portfolio_Gallery extends Widget_Base
{
	...
	protected function register_content_image_gallery_controls()
	{
		...
		$this->add_control(
			...
			[
				'label' => __('Image Lightbox Popup?', 'classyea'),
				'description' => __("Only use for design four","classyea"),
				'type' => Controls_Manager::SWITCHER,
				'default' => 'yes',
			]
		);
		...
		$repeater->add_control(
			'content_enable',
			[
				'label' => esc_html__( 'Content Enable?', 'classyea' ),
				'type' => Controls_Manager::SWITCHER,
	...
	protected function register_gallery_style_tab_controls()
	{
		...
		$this->add_control(
			'classyea_gallery_item_description_area',
			[
				'label' => esc_html__('Description', 'classyea'),
				'type' => Controls_Manager::HEADING,
			]
		...
		$this->add_control(
			'classyea_gallery_desc_normal_color',
			[
				'label'		 => esc_html__('Color', 'classyea'),
				'type'		 => Controls_Manager::COLOR,
				'selectors'	 => [
		...
		$this->add_group_control(
			Group_Control_Typography::get_type(),
			[
				'name'		 => 'classyea_gallery_description_typography',
				'selector'	 => '{{WRAPPER}} .description-text',
			]
		);
		$this->add_responsive_control(
			'classyea_gallery_descriptiion_margin',
			[
				'label' => esc_html__('Margin', 'classyea'),
				'type' => Controls_Manager::DIMENSIONS,
				'size_units' => ['px', '%', 'em'],
				'selectors' => [
					'{{WRAPPER}} .description-text' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->start_controls_section(
			'classyea_gallery_lightbox_control',
			[
				'label' => esc_html__('Lightbox Button', 'classyea'),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);

		$this->add_control(
			'classyea_lightbox_bg_color',
			[
				'label'		 => esc_html__('Background Color', 'classyea'),
				'type'		 => Controls_Manager::COLOR,
				'selectors'	 => [
					'{{WRAPPER}} .classyea-portfolio-356 .classyea-portfolio-overlay .link' => 'background: {{VALUE}};',
				]
			]
		);

		$this->add_control(
			'classyea_lightbox_text_color',
			[
				'label'		 => esc_html__('Text Color', 'classyea'),
				'type'		 => Controls_Manager::COLOR,
				'selectors'	 => [
					'{{WRAPPER}} .classyea-portfolio-356 .classyea-portfolio-overlay .link i' => 'color: {{VALUE}};',
				]
			]
		);
	...
	protected function classyea_render_gallery_repeater_control()
	{
		...
		$image_lightbox_popup  = $settings['image_lightbox_popup'];
		...
		foreach ($settings['galley_items_data'] as $key => $gallery_item) {
			...
			$content_enable  	    = $gallery_item['content_enable'];
			...
			<?php elseif ($gallery_layout == 'layout-6') : ?>
				<div class="classyea-portfolio-box-356" data-item="<?php echo esc_attr($category_arr_class[$key]); ?>">
					<?php $this->classyea_gallery_thumbnail_image($gallery_item); ?>
					<div class="classyea-portfolio-overlay">
					<?php if($image_lightbox_popup == 'yes') { ?>
					<div class="link">
						<a class="lightbox-image" href="<?php echo esc_url($item_image);?>"><i class="fas fa-plus"></i></a>
					</div>
					<?php  } ?>
						<?php $this->classyea_render_name_heading($gallery_item, $key);
						if($content_enable == 'yes') {
						   $this->classyea_description_func($gallery_item, $key);
						}
						$this->classyea_button_link_markup($gallery_item, $key); ?>
					</div>
				</div>
	...
	protected function classyea_render_name_heading($gallery_item, $key)
	{
		if ($gallery_item['item_name']) {
			...
			if ($gallery_item['item_name']) {
				$title_tag = Classyea_Helper::classyea_validate_html_tag($gallery_item['title_tag']);
		?>
	...
	protected function classyea_description_func($gallery_item, $key) {
		$content_desc_text  = $gallery_item['content_desc_text']; ?>
		<p class="description-text">
			<?php 
			echo wp_kses($content_desc_text,'classyea_kses');
			?>
		</p>
		<?php 
	}
