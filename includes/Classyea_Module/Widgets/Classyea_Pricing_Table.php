...
class Classyea_Pricing_Table extends Widget_Base
{
	...
	/**
	 * Retrieve Pricing table widget category.
	 * @access public
	 * @return string Widget icon.
	 */
	...
	**/
	protected function register_repeater_service_controls()
	{
		...
		/**
		 * Content Repeater: Pricing table
		 */
		$this->start_controls_section(
	...
	**/
	...
	protected function register_style_background_controls()
	{
		...
		$this->add_responsive_control(
            ...
            [
                ...
                'selectors'          => [
                    '{{WRAPPER}} .classyea-price-table' => 'text-align: {{VALUE}};',
                ],
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', '%'],
				'selectors'  => [
					'{{WRAPPER}} .classyea-price-table' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	/**
	 * Pricing table repeater control function
	 * Render Pricing table repeater output on the frontend.
	 * @access protected
	 */
...
?>
    <div class="classyea-price-table <?php echo esc_attr($pricing_layout.' '.$featured_class)?>">
		<div class="classyea-price-table-title-wrapper">
        	<<?php echo wp_kses_post($settings['title_tag']);?> class="classyea-price-table-title"><?php echo wp_kses($settings['title'],'classyea_kses');?></<?php echo wp_kses_post($settings['title_tag']);?>>
		</div>
        <span class="classyea-price-table-price">
			<sup><?php echo wp_kses_post($settings['currency']);?></sup> 
			<?php echo wp_kses_post($settings['price']);?>
			<?php if (!empty($settings['old_price'])) { ?>
			<sub class="period"> <?php echo wp_kses_post($settings['old_price']);?></sub>
			<?php } ?>
			<sub> /<?php echo wp_kses_post($settings['duration']);?></sub>
		</span>
        <ul>
            <?php foreach($settings['service_list'] as $service){?>
            <li><?php echo wp_kses($service['service_name'],'classyea_kses'); ?>
