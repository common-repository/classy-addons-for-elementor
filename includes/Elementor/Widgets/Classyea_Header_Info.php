...
class Classyea_Header_Info extends Widget_Base
{
    ...
    protected function register_controls()
    {
        ...
        $this->add_responsive_control(
            'classyea_simple_tab_title_icon_size',
            [
    ...
    protected function header_info_item()
    {
        ...
        $items = $settings['classyea_header_items'];
?>
        ...
        <?php
            if ( $items ) {
                foreach ( $items as $key => $item) {
                    if (!empty($item['classyea_headerinfo_link']['url'])) {
                        $this->add_link_attributes('button-' . $key, $item['classyea_headerinfo_link']);
                    }
            ?>
            <li>
                <div class="icon">
                    <?php Icons_Manager::render_icon($item['classyea_headerinfo_icons'], ['aria-hidden' => 'true']); ?>
                </div>
                <div class="text">
                    <h6>
                        <a <?php echo $this->get_render_attribute_string('button-' . $key );  ?>>
                        <?php echo esc_html($item['classyea_headerinfo_text']); ?></a></h6>
                </div>
            </li>
            <?php
