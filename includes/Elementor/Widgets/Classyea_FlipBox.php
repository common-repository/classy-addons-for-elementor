...
class Classyea_FlipBox extends Widget_Base
{
	...
	private function classyea_flip_box_repeater_control()
	{
		...
		if ($flipbox_layout == 'layout-1' || $flipbox_layout == 'layout-3' || $flipbox_layout == 'layout-4' || $flipbox_layout == 'layout-5' || $flipbox_layout == 'layout-6' || $flipbox_layout == 'layout-7' || $flipbox_layout == 'layout-8' || $flipbox_layout == 'layout-9') {
						...
						<div class="map">
							<?php 
								if($google_map_iframe != '') {
									...
									echo "<div class='map-iframe' style='padding-top:5px;padding-bottom:50px;color:red;'>Please Input the map iframe</div>";
								}
							?>
						</div>
							<?php if ('button' === $settings['link_type'] && !empty($settings['flipbox_button_text'])) { ?>
		...
		<div class="classyea-cube">
			...
			<div class="classyea-cube-default-state">
				<p class="classyea-cube-text"><?php Icons_Manager::render_icon( $cube_icon, array('aria-hidden' => 'true'));?> <strong><?php echo $total_follower;?></strong> <?php echo wp_kses($settings['flipbox_front_title'], 'classyea_kses'); ?></p>                                
			</div>
		</div>
		<?php }
...
<?php
	...
	protected function classyea_front_title_tag($settings) {
		...
		if ($settings['flipbox_front_title']) {
			$flipbox_front_title_tag = Header::classyea_validate_html_tag($settings['flipbox_front_title_tag']);
		?>
	...
	protected function classyea_back_title_tag( $settings ) {
		...
		if ($settings['flipbox_back_title']) {
			$flipbox_back_title_tag = Header::classyea_validate_html_tag($settings['flipbox_back_title_tag']);
		?>
