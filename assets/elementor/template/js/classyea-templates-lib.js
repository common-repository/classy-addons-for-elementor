;( function( elementor, $, window ) {
	...
	var TemplateLibraryInsertTemplateBehavior = Marionette.Behavior.extend({
		...
		onInsertButtonClick: function onInsertButtonClick() {
			...
			$e.run('classyea/insert-template', args);
		}
	});
	...
	var TemplateLibraryTemplateClassyEaView = TemplateLibraryTemplateView.extend({
		template: '#tmpl-elementor-template-library-template-classyea',
		ui: function ui() {
		...
		onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
			...
			classyea_templates_lib.templates.markAsFavorite(this.model, isFavorite);

			if (!isFavorite && classyea_templates_lib.templates.getFilter('favorite')) {
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-preview-classyea',
	...
	var TemplateLibraryHeaderBackView = Marionette.ItemView.extend( {
		...
		onClick: function() {
			$e.routes.restoreState( 'classyea' );
		},
	} );

	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-elementor-template-library-templates-classyea',
		...
		getChildView: function getChildView(childModel) {
			return TemplateLibraryTemplateClassyEaView;
		},
		initialize: function initialize() {
			this.listenTo(elementor.channels.templates, 'filter:change', this._renderChildren);
		},
		filter: function filter(childModel) {
			var filterTerms = classyea_templates_lib.templates.getFilterTerms(),
				passingFilter = true;
			jQuery.each(filterTerms, function (filterTermName) {
				var filterValue = classyea_templates_lib.templates.getFilter(filterTermName);
		...
		addSourceData: function addSourceData() {
			...
			this.$el.attr('data-template-source', isEmpty ? 'empty' : classyea_templates_lib.templates.getFilter('source'));
		},
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('elementor-templates-filter-active', !!(classyea_templates_lib.templates.getFilter('text') || classyea_templates_lib.templates.getFilter('favorite')));
		},
		onRender: function onRender() {
			if ('classyea' === classyea_templates_lib.templates.getFilter('source')) {
				this.setFiltersUI();
			}
		},
		onRenderCollection: function onRenderCollection() {
			...
			if ('classyea' === classyea_templates_lib.templates.getFilter('source') && 'page' !== classyea_templates_lib.templates.getFilter('type')) {
				this.setMasonrySkin();
			}
		},
		...
		onTextFilterInput: function onTextFilterInput() {
			classyea_templates_lib.templates.setFilter('text', this.ui.textFilter.val());
		},
		onSelectFilterChange: function onSelectFilterChange(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('elementor-filter');
				classyea_templates_lib.templates.setFilter(filterName, $select.val());
		},
		onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
			classyea_templates_lib.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
		},
	...
	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-templates-modal__header__logo_classyea',
		className: function className() {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-actions-classyea',
		...
		onSyncClick: function() {
			...
			classyea_templates_lib.templates.requestLibraryData({
				onUpdate: function onUpdate() {
					self.ui.sync.removeClass('eicon-animation-spin');
					$e.routes.refreshContainer('classyea');
				},
				forceUpdate: true,
				forceSync: true
			});
	...
	var TemplateLibraryHeaderMenuView = Marionette.ItemView.extend({
		...
		templateHelpers: function templateHelpers() {
			return {
				tabs: $e.components.get('classyea').getTabs()
			};
		}
	});
	...
	var TemplateLibraryLayoutView = elementorModules.common.views.modal.Layout.extend({
		...
		getTemplateActionButton: function(templateData) {
			...
			if ( templateData.isPro && 'inactive' === templateData.proStatus ) {
				viewId = '#tmpl-elementor-template-library-get-pro-button-classyea';
			}
			if ( templateData.isPro && 'license_inactive' === templateData.proStatus ) {
				viewId = '#tmpl-elementor-pro-template-library-activate-license-button-classyea';
			}
	...
	var Component = function(ComponentModal) {
		...
		Component.prototype.show = function( args ) {
			...
			if ( args.toDefault || ! $e.routes.restoreState( 'classyea' ) ) {
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.requestLibraryData = function (options) {
			...
			elementorCommon.ajax.addRequest('classyea_get_library_data', ajaxOptions);
		};
	...
	classyea_templates_lib.templates = new TemplateLibraryManager();
	classyea_templates_lib.templates.init();
	...
	elementor.on('document:loaded', function() {
		...
		var $templateLibBtnStyle = $( '<style />' ),
			btnStyle = '';
			btnStyle += '.elementor-add-section-area-button.classyea-add-template-button { margin-left: 8px; vertical-align: bottom; }';
			btnStyle += '.elementor-add-section-area-button.classyea-add-template-button img { height: 40px; }';
		...
		$templateLibBtn.addClass( 'elementor-add-section-area-button classyea-add-template-button' );
		$templateLibBtn.attr( 'title', 'Add Classy Template' );
		$templateLibBtn.html( '<img src="' + classyea_templates_lib.logoUrl + '" />' );
		...
		$previewContents.find('body').delegate('.elementor-editor-element-add', 'click', function(e) {
			setTimeout(function() {
				...
				if ( sectionInner.length > 0 ) {
					sectionInner.find( '.classyea-add-template-button' ).remove();
